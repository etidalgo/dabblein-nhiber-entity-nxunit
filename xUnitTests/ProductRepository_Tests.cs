    ...
    public class ProductRepository_Fixture
    {
        ...
        public void Can_add_new_product()
        {
            ...
            using (ISession session = _sessionFactory.OpenSession())
            {
                ...
                Assert.NotSame(product, fromDb);
                Assert.Equal(product.Name, fromDb.Name);
                Assert.Equal(product.Category, fromDb.Category);
            }
        }
        ...
        public void Can_get_existing_product_by_id()
        {
            ...
            Assert.NotNull(fromDb);
            Assert.NotSame(_products[1], fromDb);
            Assert.Equal(_products[1].Name, fromDb.Name);
        }
        ...
        public void Can_get_existing_product_by_name()
        {
            ...
            Assert.NotNull(fromDb);
            Assert.NotSame(_products[1], fromDb);
            Assert.Equal(_products[1].Id, fromDb.Id);
        }
        [Fact]
        public void Can_get_existing_products_by_category()
        {
            ...
            Assert.True(IsInCollection(_products[0], fromDb));
            Assert.True(IsInCollection(_products[1], fromDb));
        }
